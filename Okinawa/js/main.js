...
const WEATHER_LOCATIONS = [
    { id: 'naha', name: '那霸市', icon: 'fa-city', prefCode: '471000', weeklyArea: '471000', shortArea: '471010', tempCity: '91197' },
    { id: 'nago', name: '名護市（北部騎乘區）', icon: 'fa-motorcycle', prefCode: '471000', weeklyArea: '471000', shortArea: '471020', tempCity: '91107', note: '氣象廳週間預報（4-7天）僅公布沖繩本島整體一份，未單獨提供名護市氣溫，第4天後氣溫可能顯示空白，屆時請以那霸市資料概略推估。' }
];
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setupCountdown();
        setupWeatherWidget('naha-weather-info', 'Naha', false);
    setupWeatherWidget('nago-weather-info', 'Nago', false);
    ...
    function generateNavButtons() {
        ...
        buttonsHtml += `<button data-target="overview" class="nav-btn bg-teal-500 text-white py-2 px-4 rounded-full shadow-sm text-center leading-tight flex-shrink-0 whitespace-nowrap">旅程總覽</button>`;
        ...
        for (const dayId in itineraryData) {
            ...
            if (dayData.navInfo) {
                ...
                const todayBadge = isToday
                    ...
                    : '';
                buttonsHtml += `
                    <button data-target="${dayId}" class="nav-btn relative ${isToday ? 'ring-2 ring-red-400 ring-offset-2' : ''} bg-teal-100 text-teal-800 py-2 px-4 rounded-full shadow-sm text-center leading-tight flex-shrink-0 whitespace-nowrap">
                        ${todayBadge}
            ...
            .catch(() => {
                ...
                weatherInfo.innerHTML = `
                    <div class="text-center w-full">
                        ${errorMessage}
                        <button id="${retryButtonId}" class="bg-teal-500 text-white py-1 px-4 rounded-full hover:bg-teal-600 transition-colors text-sm">重試</button>
                    </div>`;
    ...
    function renderWeatherLocationCard(location, today, days) {
        ...
        const boxes = days.map(d => {
            ...
            const sourceBadge = d.source === 'short'
                ...
                : `<span class="text-[8px] sm:text-[9px] leading-none px-1 py-0.5 rounded bg-cyan-100 text-cyan-600">週間</span>`;
            return `
    ...
    function setupInteractiveForeground() {
        ...
        // 沖繩10月還是亞熱帶氣候，不是楓葉季，改用扶桑花（沖繩縣花，全年常開）色調
        const hibiscusColors = [
            'rgba(225, 29, 72,',  // Deep pink/red
            'rgba(251, 113, 133,', // Coral pink
            'rgba(253, 224, 71,', // Sun-glint yellow
            'rgba(45, 212, 191,', // Ocean teal
        ];
        ...
        class Particle {
            constructor() {
                ...
                this.color = hibiscusColors[Math.floor(Math.random() * hibiscusColors.length)];
            }
            update() {
                this.y += this.speedY;
                this.x += this.speedX;
            ...
            draw() {
                ...
                ctx.fillText('🌺', 0, 0); // 扶桑花，符合沖繩10月的實際季節
                ctx.restore();
            }
        }
    ...
    function setupCountdown() {
        ...
        const interval = setInterval(() => {
            ...
            if (distance < 0) {
                ...
                countdownElement.innerHTML = "<div class='col-span-4 text-2xl font-bold text-teal-600'>旅程已開始！</div>";
                return;
            }
            ...
            const createCardHtml = (planData) => {
                ...
                const timelineHtml = planData.items.map(item => `
                            <div class="timeline-item relative mb-8">
                                ...
                                <div class="ml-10 sm:ml-8 bg-white rounded-lg shadow-md overflow-hidden card-hover">
                                    <div class="p-4">
                                        <h3 class="text-base sm:text-lg font-bold text-teal-700">${item.time} | ${item.title}</h3>
                                    </div>
                ...
                return `
                            <div class="daily-theme-card cursor-pointer relative rounded-2xl shadow-xl overflow-hidden p-8 md:p-12 text-white flex flex-col justify-center items-center text-center min-h-[300px] mb-12" data-bg-src="${planData.themeImage}" style="background-image: linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5));">
                                ...
                                ${highlightsHtml}
                                <button class="view-timeline-btn mt-6 bg-teal-600 text-white py-2 px-6 rounded-full shadow-lg hover:bg-teal-700 transition-transform hover:scale-105 flex items-center justify-center">查看詳細行程 <i class="fas fa-chevron-down ml-2"></i></button>
                            </div>
    ...
    function switchTab(targetId, shouldScroll = true, onCompleteCallback = () => {}) {
         ...
         tl.add(() => {
             navButtons.forEach(btn => {
                 ...
                 btn.classList.toggle('bg-teal-500', isTarget);
                 btn.classList.toggle('text-white', isTarget);
                 btn.classList.toggle('bg-teal-100', !isTarget);
                 btn.classList.toggle('text-teal-800', !isTarget);
                 if (isTarget) {
